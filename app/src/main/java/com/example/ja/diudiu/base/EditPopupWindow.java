package com.example.ja.diudiu.base;

import android.content.Context;
import android.view.LayoutInflater;
import android.view.View;
import android.widget.TextView;

import com.example.ja.diudiu.R;
import com.example.ja.diudiu.i.IPopupItemClick;

/**
 * Created by JA on 2015/7/14.
 */
public class EditPopupWindow extends BasePopupWindow implements View.OnClickListener {

    private TextView mEdit;
    private TextView mDelete;
    private IPopupItemClick mOnPopItemClickListener;

    public EditPopupWindow(Context context, int width, int height) {
        super(LayoutInflater.from(context).inflate(R.layout.pop_device, null), dpToPx(context, width),
                dpToPx(context, height));
        setAnimationStyle(R.style.PopupAnimation);
    }

    private static int dpToPx(Context context, int dp) {
        return (int) (context.getResources().getDisplayMetrics().density * dp + 0.5f);
    }

    @Override
    public void initEvents() {
        mEdit.setOnClickListener(this);
        mDelete.setOnClickListener(this);
    }

    @Override
    public void initViews() {
        mEdit = (TextView) findViewById(R.id.tv_pop_edit);
        mDelete = (TextView) findViewById(R.id.tv_pop_delete);
    }

    @Override
    public void init() {

    }

    @Override
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.tv_pop_edit:
                if (mOnPopItemClickListener != null) {
                    mOnPopItemClickListener.onEdit(v);
                }
                break;
            case R.id.tv_pop_delete:
                if (mOnPopItemClickListener != null) {
                    mOnPopItemClickListener.onDelete(v);
                }
                break;
        }
        dismiss();

    }
    public void setmOnPopItemClickListener(IPopupItemClick l) {
        mOnPopItemClickListener = l;
    }
}
